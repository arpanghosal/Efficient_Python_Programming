...
__global__ void multiply_square_matrices(const int size, const float *A,
    ...
    // to do: use strided-loop to optimise your strided-loop
...
int main(int argc, char *argv[]) {
  ...
  // to do: modify here threads and blocks number
  // int number_of_threads = 32;
  // to make a simple loop use:
  // int number_of_blocks = (size + number_of_threads - 1) / number_of_threads;
  // otherwise make a strided-loop and you can define any grid size
  dim3 grid(1);
  dim3 block(1);
