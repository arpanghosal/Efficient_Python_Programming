...
__global__ void vector_addition_gpu(int *a, int *b, int *c, int size) {
  ...
  /* Step 2 to do: modify the for loop such that it is executed in parallel
     Look into the slides of the lecture for details
  ...
  printf('Doing sum conventionally');
  for (int i = start; i < size; i += stride) { 
    c[i] = a[i] + b[i];
  }
  */
  
  //printf('Doing sum using parallel methods');
  const int start = threadIdx.x + blockIdx.x * blockDim.x;
  const int stride = blockIdx.x * gridDim.x;
  for (int i = start; i< size; i+=stride)
  {
    //printf('Adding elements %u', threadIdx.x + blockIdx.x * blockDim.x);
    c[i] = a[i] + b[i];      // See slide 49 of presentation
...
int main(int argc, char *argv[] ) {
   ...
   */
    ...
    cout << "copy from host to device complete"<<endl;
  ...
  */
   ...
   */
   cout << "Gonna start adding now"<<endl;
   vector_addition_gpu<<<grid_dim, block_dim>>>(a_d, b_d, c_d, size);
   
  /* Copy result vector from device to host */
  /* Step 3 to do: Copy content of c_d to c_h 
     Follow the syntax of cudaMemcpy from above, but remember that we want to copy from the device to the host now
   */
   
    cudaMemcpy( c_h, c_d, size * sizeof(int), cudaMemcpyDeviceToHost );
    cout << "copying back result from device to host"<<endl;
  ...
  /* Step 3 to do: Uncomment the call to check_elements to verify the vector addition result from the GPU  */
   cout << "checking the elements now" << endl;
  ...
  /* Step 1 to do: Free device memory
     ...
     Do the same for b_d and c_d!
   */
   cout << "Freeing out the memory"<<endl;
    cudaFree( a_d );
    cudaFree( b_d );
    cudaFree( c_d );
